...
In this work, we investigate whether attention functions as a unifying bottleneck across AI and Internet systems. We compare attention-based architectures against non-attention baselines in two representative tasks: \sectask using MovieLens and \ctrtask using the Criteo dataset. Our approach focuses on not just the raw performance metrics but also the underlying attention concentration patterns. We demonstrate that attention-based models are not only competitive but consistently provide superior regularization and predictive accuracy in these high-dimensional domains.
...
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=0pt]
    \item We demonstrate that the attention-based paradigm consistently matches or outperforms non-attention baselines in critical Internet tasks, specifically \sectask and \ctrtask.
